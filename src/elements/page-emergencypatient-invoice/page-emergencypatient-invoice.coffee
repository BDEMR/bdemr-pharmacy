...
Polymer {
  is: 'page-emergencypatient-invoice'
  ...
  _getDisabledDivClassesForCategoryAndInvoice: (patientExists)->
    if patientExists
      return "card-custom-header custom-input-field disabledbutton"
    else
      "card-custom-header custom-input-field"

  _getDisabledDivClassesForPatient: (patientExists)->
    if patientExists
      return "card-custom-header custom-input-field disabledbutton"
    else
      "card-custom-header custom-input-field"
  ...
  _searchPatientByPhone: (searchQuery)->
    ...
    @callApi '/bdemr-patient-search-new', data, (err, response)=>
      ...
      if response.hasError
        ...
        @patientExists = true if matchingPatientList.length > 0
        console.log 'matching patient list', matchingPatientList
  ...
  signupPatientAndProcessInvoice: (cbfn)->
    # first go through patient signup
    console.log 'signing up patient first'
  ...
  updateInvoiceObjectsWithNewPatient: (newPatient)->
    @set 'invoice.patientSerial', newPatient.serial
