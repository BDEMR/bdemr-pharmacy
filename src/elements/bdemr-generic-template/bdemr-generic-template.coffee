...
Polymer {
  ...
  _setFormattedTemplateContent: (templateContent)->
    this.$$('.editor-content').innerHTML = templateContent
  ...
  editTapped: (e = null)->
    localStorage.setItem 'to_generic_template', JSON.stringify true
    window.editor.toEdit = @templateData.templateContent
    localStorage.setItem 'from_generic_template', JSON.stringify true
    @domHost.domHost.navigateToPage '#/editor'
  ...
  templatePressed: (e)->
    ...
    @set 'templateData', {templateName: template.name, templateContent: template.content}
    @_setFormattedTemplateContent @templateData.templateContent
    # this._assertTemplateContent();
