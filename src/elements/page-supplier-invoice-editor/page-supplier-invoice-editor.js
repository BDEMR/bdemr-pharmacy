...
Polymer({
  is: 'page-supplier-invoice-editor',
  behaviors: [
  ...
  navigatedIn() {
    ...
    if (params['organization']) {
      this._loadOrganization(params['organization'], () => {
        ...
        this._loadPharmacySupplierCategories(params['organization'])
        this._makeNewItem()
      });
    } else {
  ...
  _loadPharmacySupplierCategories(organizationIdentifier) {
    // let list = app.db.find('pharmacy-supplier-invoice-categories', ({ organizationId }) => organizationId === organizationIdentifier);
    // if (list.length) {
    ...
    this.callApi('/bdemr--clinic-pharmacy-supplier-invoice-category-list', data, (err, response) => {
      if (response.hasError) return this.domHost.showModalDialog(response.error.message);
      else {
        let pharmacySupplierCategoryList = response.data
        this.set('expenseCategories', pharmacySupplierCategoryList);
  ...
  expenseCategoryCustomSet(e) {
    ...
    if (!found) {
      let newCategory = {
        serial: this.generateSerialForSupplierInvoice(),
        createdDatetimeStamp: lib.datetime.now(),
        createdByUserSerial: this.user.serial,
        organizationId: this.organization.idOnServer,
      ...
      app.db.upsert('pharmacy-supplier-invoice-categories', newCategory, ({ serial }) => serial === newCategory.serial);
  ...
  saveButtonClicked() {
    ...
    if (!this.expense.serial) this.expense.serial = this.generateSerialForSupplierInvoice();
    this.expense.lastModifiedDatetimeStamp = Date.now()
    app.db.upsert('pharmacy-supplier-invoice', this.expense, ({ serial }) => serial === this.expense.serial);
